...
BOSS.HeaderHandle = {
    ...
    Menu : {
        init : function () {
            ...
            //获取数据列表
            UseHeaderHandle.Menu.getListData(BOSS.api.getChannelUserList + '?requestExtend=' + UseHeaderHandle.requestExtend, filter);
            //获取级联市
            ...
            //更改用户状态
            UseHeaderHandle.changeStatus.on('show.bs.modal', function (event) {
        ...
        getListData : function (requestLink, data) {
            BOSS.HeaderHandle.getAjax(requestLink, data,function (d) {
                if(d.data != null){
                    BOSS.HeaderHandle.channelUserListTemplate.draw(d.data.data);
                    $('.panel-footer').html(d.data.pageInfo);
                    $('.orders-num').text(d.data.num);
                } else {
                    BOSS.HeaderHandle.channelUserListTemplate.draw();
                    $('.orders-num').text(0);
     ...
     * 获取待渠道用户列表模板
     */
    channelUserListTemplate : {
        draw : function(data){
            var listStr = '',listDom, list = $('.panel-body-table table tbody'), item = $('tr', list);
            // 引用 View/ChannelMember/channelUserListTemplate.html
            $.get('http://' + BOSS.domain + '/ChannelMember/channelUserListTemplate.html',function(template) {
                if (item.length) {
                ...
                if(!BOSS.valid.checkEmptyObj(data)){
                    for (var i=0; i<data.length; i++){
                        ...
                        data[i].freal_name = BOSS.string.replaceHTML(data[i].freal_name);
                        ...
                        if(data[i].fis_locked==1){
                            data[i].flocked_name = "<font color='red'>已锁定</font>";
                            data[i].flocked_name_title = '解锁';
                        }else{
                            data[i].flocked_name = "未锁定";
