...
BOSS.Inventory = {
    ...
    Menu : {
        init : function () {
            ...
            //获取库存列表
            Inventory.Menu.getInventorys(BOSS.api.ajaxTransferList, filter);
            ...
            //出库
            Inventory.productOutside.on('show.bs.modal', function (event) {
            ...
            //单个商品调拨
            Inventory.productTransfer.on('show.bs.modal', function (event) {
                var model = $(this), button = $(event.relatedTarget), inventoryId = button.data('id'),
                    validateParams = {};
                model.find('[name="inventoryId"]').attr('value', inventoryId);
            ...
            //批量调拨
            Inventory.productTransferWhere.on('show.bs.modal', function () {
