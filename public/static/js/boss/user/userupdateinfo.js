    ...
    Menu : {
        init : function () {
            ...
            var login_page_id = $('input[name="hid_flogin_page_id"]').val(),
                mchid = $('input[name="hid_fmchid"]').val(),
            ...
            //部门mchid
            if(mchid != ""){
                $('select[name="Fmchid"] option[value="' + mchid +'"]').attr("selected", true);
            };
            //省
            if(province_id != ""){
            ...
            $('body').on('click','#sub_frm', function () {
                var model = UserUpdateInfo.myFrm,Fmchid = $("select[name='Fmchid']"),Frole_id = $("select[name='Frole_id']"),
                ...
                var selectEl = {"Fmchid":Fmchid,"Frole_id":Frole_id,"Farea_id":Farea_id,"Flogin_page_id":Flogin_page_id};
            ...
            $parentBox.validate({
                ...
                submitHandler : function () {
                    // if($parentBox.hasClass('form-visit')){
                        BOSS.UserUpdateInfo.getAjax(BOSS.api.saveUserInfo, $parentBox.serialize(), function (d) {
                            if(d.data){
                                BOSS.floatTips.successTips('信息保存成功!');
                                setTimeout(function () {
                                    window.location.href = BOSS.api.userList;
                                }, 1000);
                            } else {
